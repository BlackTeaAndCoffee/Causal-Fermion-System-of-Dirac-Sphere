...
def Diag_plot(x_values, y_matrix, X_Title, Y_Title, PDFTitle):
    ...
    c = graph.graphxy(width=10,height=10,
                      x = graph.axis.linear(min = min(x_values), max = max(x_values), title= X_Title),
                      y = graph.axis.linear(min = np.amin(y_matrix), max = np.amax(y_matrix), title= Y_Title))
...
def Lagrange_Schar(a, x_values, y_matrix, kappa):
...
if __name__ == "__main__":
    ...
    k_list=np.linspace(0.3,0.5,K_Anzahl)
    a = ClosedChain(t, r, 0, 0, N)

    #print simplify(Lagrangian_without_Max(t,r,0,0,N))
    #lagr = lambdify((t,r), simplify(Lagrangian_without_Max(t,r,0,0,N)))

    dd =  (trace(a)**2 + trace(a*a)**2)/2
    print simplify(dd)
    #ddl = lambdify((t,r), dd)
    kappa = 0.1
    ...
    y_values=[]
#    for i in range(L):
#        y_values.append(ddl(t,x_values[i]))

#    db.f_gegn_x(list(x_values), y_values, 'radius', 'Lagr', 'test')

    #Lagrange_Schar(a, x_values, y_Matrix, kappa)
